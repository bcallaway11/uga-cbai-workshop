...
<span class="alert">Additional Workshop Materials: </span> [https://github.com/bcallaway11/uga-cbai-workshop](https://github.com/bcallaway11/uga-cbai-workshop)
...
* For today: very simplified version with (1) no changes in federal minimum wage and (2) "binarized" state minimum wages (i.e., state minimum wage is either above or below the federal minimum wage)
...
\begin{align*}
  Y_{i,t=2} = D_i Y_{i,t=2}(1) +(1-D_i)Y_{i,t=2}(0) \quad \textrm{and} \quad Y_{i,t=1} = Y_{i,t=1}(0)
\end{align*}
...
$$ATT = \E[Y_{i,t=2}(1) - Y_{i,t=2}(0) | D=1]$$
...
Notice that:
\begin{align*}
  ATT = \underbrace{\E[Y_{i,t=2}(1) | D=1]}_{\textrm{Easy}} - \underbrace{\E[Y_{i,t=2}(0) | D=1]}_{\textrm{Hard}}
\end{align*}
...
\begin{align*}
  ATT = \color{green}{\E[Y_{i,t=2}(1) - Y_{i,t=1}(0) | D=1]} - \color{red}{\E[Y_{i,t=2}(0) - Y_{i,t=1}(0) | D=1]}
\end{align*}
...
* There are many possibilities here:
    1. <span class="alert-blue">Before-after: </span> $\color{red}{\E[Y_{i,t=2}(0) - Y_{i,t=1}(0) | D=1]} = 0$
...
* There are many possibilities here:
    2. <span class="alert-blue">Lagged outcome unconfoundedness: </span> $\color{red}{\E[Y_{i,t=2}(0) - Y_{i,t=1}(0) | D=1]} = \E\Big[ \E[Y_{i,t=2}(0) | Y_{i,t=1}, D=0] - Y_{i,t=1}(0) \Big| D=1\Big]$
...
* There are many possibilities here:
    3. <span class="alert-blue">Change-in-changes: </span> $\color{red}{\E[Y_{i,t=2}(0) - Y_{i,t=1}(0) | D=1]} = \E\Big[ Q_{Y_{i,t=2}(0)|D=0}\big(F_{Y_{i,t=1}(0)|D=0}(Y_{i,t=1}(0))\big) - Y_{i,t=1}(0) \Big| D=1\Big]$
...
$$\\color{red}{\\E[\\Delta Y_i(0) | D=1]} = \\E[\\Delta Y_i(0) | D=0]$$
...
eqlist <- list("ATT &= \\E[\\Delta Y_i | D=1] - \\E[\\Delta Y_i(0) | D=1]", 
               "&= \\E[\\Delta Y_i | D=1] - \\E[\\Delta Y_i | D=0]")
...
$$\widehat{ATT} = \frac{1}{n_1} \sum_{i=1}^n D_i \Delta Y_i - \frac{1}{n_0} \sum_{i=1}^n (1-D_i) \Delta Y_i$$
...
\begin{align*}
  \alpha = \E[\Delta Y_{i,t}|D_{i,t}=1] - \E[\Delta Y_{i,t}|D=0] = ATT
\end{align*}
...
* $D_{i,t}$ &mdash; treatment indicator.  
...
\begin{align*}
  ATT(g,t) = \E[ \tau_{i,t}(G) | G_i=g] = \E[Y_{i,t}(g) - Y_{i,t}(0) | G_i=g]
\end{align*}

<p style="margin-left: 20px;">Explanation: $ATT$ for group $g$ in time period $t$</p>
...
<p style="margin-left: 20px;">where $\mathcal{G}_e$ is the set of groups observed to have experienced the treatment for $e$ periods at some point.</p>

<p style="margin-left: 20px;">Explanation: $ATT$ when units have been treated for $e$ periods</p>
...
<p style="margin-left: 20px;">Explanation: $ATT$ across all units that every participate in the treatment</p>
...
[Overall ATT]{.alert}
\begin{align*}
  ATT^o = \sum_{g \in \bar{\mathcal{G}}} \sum_{i,t=g}^{\T} w^o(g,t) ATT(g,t)
\end{align*}
where
\begin{align*}
...
\begin{align*}
\alpha &= \sum_{g \in \bar{\mathcal{G}}} \sum_{i,t=g}^{\T}  w^{TWFE}(g,t) \Big( \E[(Y_{i,t} - Y_{i,g-1}) | G_i=g] - \E[(Y_{i,t} - Y_{i,g-1}) | U_i=1] \Big) \\
  & + \sum_{g \in \bar{\mathcal{G}}} \sum_{i,t=1}^{g-1} w^{TWFE}(g,t) \Big( \E[(Y_{i,t} - Y_{i,g-1}) | G_i=g] - \E[(Y_{i,t} - Y_{i,g-1}) | U_i=1] \Big)
\end{align*}
...
\begin{align*}
\alpha = \sum_{g \in \bar{\mathcal{G}}} \sum_{i,t=g}^{\T} w^{TWFE}(g,t) ATT(g,t)
\end{align*}
...
\begin{align*}
  ATT^{simple} := \sum_{i,t=g}^\T ATT(g,t) \frac{\P(G_i=g | G \in \bar{\mathcal{G}})}{\sum_{i,t=g}^{\T} \P(G_i=g| G \in \bar{\mathcal{G}})}
\end{align*}
